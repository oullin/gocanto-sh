...
function ringFor(row: number, col: number): Ring {
    const key = `${row},${col}`;
    if (key in OUTER_ORDER) {
        return "outer";
    }
    if (key in MIDDLE_ORDER) {
        return "middle";
    }
    return "center";
}
...
function orderFor(row: number, col: number, ring: Ring): number {
    const key = `${row},${col}`;
    if (ring === "outer") {
        return OUTER_ORDER[key]!;
    }
    if (ring === "middle") {
        return MIDDLE_ORDER[key]!;
    }
    return 0;
}
</script>
...
@keyframes twin-orbit-ring {
    ...
    20% {
        opacity: calc(
            0.45 * var(--opacity-peak) + 0.45 * var(--opacity-mid) + 0.1 * var(--opacity-base)
        );
    }
    30% {
        opacity: calc(
            0.2 * var(--opacity-peak) + 0.4 * var(--opacity-mid) + 0.4 * var(--opacity-base)
        );
    }
    40% {
