...
import { bankingPayload } from "@features/workflow-hero/data/banking";
...
describe("WorkflowHero deferred panels", () => {
    it("renders the default workflow without a loading transition", () => {
        ...
        expect(wrapper.text()).toContain("Ollin Agent Go");
    ...
    it("shows a contained error and retries a rejected chunk load", async () => {
        ...
        const wrapper = mount(WorkflowHero, { props: { workflowLoader } });
        ...
        expect(wrapper.text()).toContain("Legacy Surface");
    });
});
