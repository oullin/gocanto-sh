...
import type { VitalsEnvironment } from "#app/lib/vitals-injector";

type Listener = EventListenerOrEventListenerObject;

class BrowserHarness {
    readonly loader = vi.fn(async () => Promise.resolve());
    readonly addWindowEventListener = vi.fn((type: string, listener: Listener) =>
        this.windowListeners.set(type, listener),
    );
    readonly removeWindowEventListener = vi.fn();
    readonly addDocumentEventListener = vi.fn((type: string, listener: Listener) =>
